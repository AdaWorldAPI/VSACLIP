...
**Rust ONLY. No Python. No numpy. EVER.**

ALL code is Rust. CLIP embeddings were computed in Rust (fastembed-rs crate).
SimHash, sweep, cascade, Belichtungsmesser, embedding — everything is Rust.
If a Rust crate is broken, fix it or find another Rust crate.
Do NOT write Python. Do NOT suggest Python. Do NOT create .py files.
...
### 3. CLIP float embeddings are a pre-processing step — IN RUST

The pipeline is:
1. **Pre-process (once, in Rust)**: CLIP ViT-B/32 generates float32[512] per image
   - Uses fastembed-rs crate or equivalent Rust CLIP inference
...
2. **Everything else is Rust**: SimHash, POPCNT sweep, cascade, evaluation
   ...
   - The entire pipeline is Rust + SIMD, zero Python
...
- Solution: only commit containers.bin + labels, not embeddings.bin
- Embeddings can be regenerated by re-running the Rust embedding binary

### 5. Commit everything

All files should be committed:
- `src/bin/poc.rs` (the main pipeline binary)
- `.gitignore`
...
```
Image -> CLIP float32[512] (Rust, one-time) -> SimHash (Rust/rayon) -> Container[u64;128] -> HDR POPCNT Sweep (Rust/SIMD) -> Recognition
```

Everything is Rust. No Python anywhere in the pipeline.
...
**Loading in Rust:**
Use `src/bin/download_data.rs` to download and extract, then
`src/ingest.rs` to scan images and compute CLIP embeddings via fastembed-rs.
...
No training. No GPU. No Python. Pure Rust.
```

## Anti-Patterns — DO NOT

- Do NOT use cosine similarity in the hot path
- Do NOT use numpy or Python for ANYTHING — not even "one-time" scripts
